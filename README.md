...
需要在“开发者工具”中重置数据库时，请在 PowerShell 中先设置本次启动使用的管理员令牌：
管理员令牌只使用英文字母、数字和常见符号；不要输入换行，也不要在开头或结尾留空格。
...
打开“开发者工具”后，输入与启动时相同的值。服务端未设置该变量时，写操作会被禁用并返回 503。`start.ps1` 不会打印令牌，也不会把令牌写入前端构建包。
