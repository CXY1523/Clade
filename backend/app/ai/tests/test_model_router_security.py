...
def router() -> ModelRouter:
    ...
    router = ModelRouter(api_key="key+&=", runtime_client=runtime_client)
...
def test_provider_request_location_for_google_escapes_model_and_key(
    ...
    assert request_target == (
        f"/models/{quote(model_name, safe='')}:{action}?{urlencode({'key': api_key})}"
    )
    assert model_name not in request_target
    assert api_key not in request_target
