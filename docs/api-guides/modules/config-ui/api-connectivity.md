...
可以把 Embedding 缓存理解为一组贴有来源标签的抽屉。v2 缓存不会只看“服务商类型和模型名称”，还会区分结果来自真实远程服务还是本地假向量，并为远程地址计算不可逆的 SHA-256 指纹：
...
- 磁盘缓存命中前会核对来源、完整命名空间和端点指纹。metadata 不保存原始 endpoint、API Key 或请求目标；端点指纹不能还原为原始地址。
